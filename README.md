...
| <a name="input_schedules"></a> [schedules](#input\_schedules) | The schedules block as documented below. | <pre>object({<br/>    days_to_build              = list(string)<br/>    schedule_only_with_changes = optional(bool)<br/>    start_hours                = optional(number)<br/>    start_minutes              = optional(number)<br/>    time_zone                  = optional(string)<br/>    branch_filter = optional(object({<br/>      include = optional(list(string))<br/>      exclude = optional(list(string))<br/>    }))<br/>  })</pre> | `null` | no |
